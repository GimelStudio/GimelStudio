...
class ImageViewportPanel(PanelBase):
    ...
    def UpdateViewerImage(self, image):
        self.imageviewport.UpdateViewerImage(image)
...
class ImageViewport(glcanvas.GLCanvas):
    def __init__(self, parent):
        ...
        self.img = np.zeros((200, 200, 4), dtype=np.float32)
    ...
    def DrawContext(self):
        ...
        paint = skia.Paint()
        image = skia.Image.fromarray(self.img.astype("uint8"))
        x = (self.Size[0] - self.img.shape[1]) / 2.0
        y = (self.Size[1] - self.img.shape[0]) / 2.0
        rect = skia.Rect.MakeXYWH(x, y, self.img.shape[1], self.img.shape[0])
        self.canvas.drawImageRect(image, rect, paint)

        self.surface.flushAndSubmit()

    def UpdateViewerImage(self, image):
        self.img = cv2.cvtColor(image, cv2.COLOR_BGRA2RGBA)
        self.Refresh(False)
